...
def compute_current_voltage_propagators(tau_s, tau_m, C_mem, staticparams):
    t0 = (
        tau_m
        * tau_s
        * (jnp.exp(-staticparams.dt / tau_m) - jnp.exp(-staticparams.dt / tau_s))
    )
...
def cg_escape(u, noise, C_m, tau_m, RI, tau_noise, V_th):
    k = tau_m / tau_noise
    sigma_u = 1.0 / jnp.sqrt(1.0 + k)
    R = tau_m / C_m
    T = ((1 + k) / 2) ** 0.5 * (V_th - u) / (R * noise)
    U_dot = (-u + RI) / tau_m
    T_dot = -1.0 / (R * noise * 2.0 ** 0.5 * sigma_u) * U_dot
    F = (2.0 / jnp.pi) ** 0.5 * jnp.exp(-(T ** 2.0)) / (1.0 + erf(T))
    B = -(2.0 ** 0.5) * tau_m * jnp.clip(T_dot, a_max=0.0) * F
    A_inf = jnp.exp(
        6.1e-3 - 1.12 * T - 0.257 * T ** 2 - 0.072 * T ** 3 - 0.0117 * T ** 4
    )
    A = A_inf * (1.0 - (1 + k) ** (-0.71 + 0.0825 * (T + 3.0)))
    lam = (A + B) / tau_m
    return lam
...
def update_state(
    ...
    s0 = params.w[staticparams.synport[0]].dot(spikes[staticparams.synport[0]])
    s1 = params.w[staticparams.synport[1]].dot(spikes[staticparams.synport[1]])
