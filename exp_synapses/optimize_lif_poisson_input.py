...
from mesogif_exp_jax import Params, StaticParams, State, integrate_log_prob, update_state, \
    compute_current_propagators, compute_current_voltage_propagators, compute_voltage_propagators, \
    simulate
import pickle
...
J = 10.
rate = 6562.5
mu_ext = tau_syn * rate / 1000. * J # divide by factor 1000. because rate is given in s^-1 instead of ms^-1
sigma2_ext = tau_syn * rate / 1000. * J**2

sigma_noise = np.sqrt(sigma2_ext/2.) # See Eq. 23 in note
...
N[0] = 4000
N[1] = 1000
params = Params(RI = jnp.ones(M, dtype=jnp.float32)*15.75,
                tau_m = jnp.ones(M, dtype=jnp.float32)*20.,
                N = jnp.array(N, dtype=jnp.float32),
                tau_s = jnp.array(tau_s),
                u_thr = jnp.ones(M, dtype=jnp.float32) * 15.,
                #c = jnp.ones(M, dtype=jnp.float32) * 0.,
                #delta_u = jnp.ones(M, dtype=jnp.float32) * .5,
                C_mem = jnp.ones(M, dtype=jnp.float32) * 250.,
                w = jnp.array(w)
                )

rec = simulate(params, staticparams, K, steps, record=['u', 'h', 'y', 'spikes', 'm', "x", "lambd_old"])
...
spikes = np.load('example_hists.npy').astype(np.float32)[500:]
# 
...
#     return paramlist
# ps = optimize(params, spikes, m_init)
# RIs = np.array([p.RI.to_py() for p in ps])
# 
# #################################
...
# 
# nparams = ps[-1]
# nrec = simulate(nparams, staticparams, K, steps, record=['u', 'h', 'y', 'spikes', 'm', 'x', 'lambd_old'])
# 
