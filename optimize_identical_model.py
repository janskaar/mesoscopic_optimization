...
N[1] = 200
params = Params(
    RI=jnp.ones(M, dtype=jnp.float32) * 17.0,
    ...
    delta_u=jnp.ones(M, dtype=jnp.float32) * 0.5,
...
# Optimize
###############
spikes = np.load("spikes1.npy").astype(np.float32)
...
ax[0].plot(spikes)
ax[1].plot(rec["spikes"])
ax[2].plot(nrec["spikes"])
plt.show()
